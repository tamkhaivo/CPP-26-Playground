...
// ----------------------------------------------------------------------------------
// CUSTOM JOLT JOB SYSTEM BACKED BY ENKITS TASK SCHEDULER
// ----------------------------------------------------------------------------------
...
protected:
    virtual void QueueJob(Job* inJob) override {
        // Execute via enkiTS task set or direct execution
        inJob->Execute();
    }
...
int main() {
    ...
    std::cout << "===============================================================\n";
    std::cout << "     OPEN-SOURCE TASK QUEUE EXPLORATION: enkiTS TASK SCHEDULER  \n";
    std::cout << "===============================================================\n";
    ...
    std::cout << "[Checklist 5] Task Scheduler: enkiTS TaskSet Scheduler (" << num_threads << " threads)\n";
    ...
    // -------------------------------------------------------------
    // RUN 1: Pure Jolt + enkiTS Task Scheduler Pass A
    // -------------------------------------------------------------
    std::cout << "\n[RUN 1] Pure Jolt + enkiTS Task Scheduler Pass A (15,000 bodies, 500 frames)...";
    ...
    // -------------------------------------------------------------
    // RUN 2: Pure Jolt + enkiTS Task Scheduler Pass B (Re-verification)
    // -------------------------------------------------------------
    std::cout << "[RUN 2] Pure Jolt + enkiTS Task Scheduler Pass B (Re-verification)...";
    JPH::TempAllocatorImpl temp_allocator_det2(32 * 1024 * 1024);
    JoltEnkiTSJobSystem job_system_det2(JPH::cMaxPhysicsJobs, JPH::cMaxPhysicsBarriers, &ts);
    ...
    // -------------------------------------------------------------
    // RUN 3: Jolt + Highway SIMD + enkiTS Parallel Range Slicing
    // -------------------------------------------------------------
    std::cout << "[RUN 3] Jolt + Highway SIMD + enkiTS TaskSet Partition Pass...";
    ...
    for (int frame = 0; frame < NUM_FRAMES; ++frame) {
        // Highway SIMD pre-pass using enkiTS deterministic range partitioning
    ...
    std::cout << "Pure Jolt + enkiTS Execution Time  : " << duration_det1 << " ms (" << duration_det1 / NUM_FRAMES << " ms/frame)\n";
    std::cout << "Jolt + Highway SIMD + enkiTS Time  : " << duration_hwy << " ms (" << duration_hwy / NUM_FRAMES << " ms/frame)\n";
    ...
    std::cout << "===============================================================\n";
    std::cout << "Pure Jolt + enkiTS Pass A Hash : 0x" << std::hex << std::uppercase << hash_det1 << std::dec << "\n";
    std::cout << "Pure Jolt + enkiTS Pass B Hash : 0x" << std::hex << std::uppercase << hash_det2 << std::dec << "\n";
    std::cout << "Jolt + Highway + enkiTS Hash   : 0x" << std::hex << std::uppercase << hash_hwy << std::dec << "\n";
