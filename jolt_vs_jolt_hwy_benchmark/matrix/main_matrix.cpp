...
int main() {
    ...
    std::cout << "===============================================================\n";
#if defined(QUEUE_ENKITS)
    std::cout << " TASK QUEUE ARCHITECTURE: enkiTS (TaskSet Range Partitioning) \n";
#elif defined(QUEUE_BARRIER)
    std::cout << " TASK QUEUE ARCHITECTURE: JobSystemWithBarrier (FIFO Barrier Queue) \n";
#elif defined(QUEUE_THREADPOOL)
    std::cout << " TASK QUEUE ARCHITECTURE: JobSystemThreadPool (Dynamic Work-Stealing) \n";
#endif

#if defined(FP_MODE_STRICT)
    std::cout << " COMPILER FP MODE       : MSVC /fp:strict (Strict IEEE 754)    \n";
#elif defined(FP_MODE_FAST)
    std::cout << " COMPILER FP MODE       : MSVC /fp:fast (Fast Math)             \n";
#endif
    std::cout << "===============================================================\n";
    ...
    // Initialize selected job system
    JPH::TempAllocatorImpl temp_allocator(32 * 1024 * 1024);
    JPH::JobSystem* job_system = nullptr;

#if defined(QUEUE_ENKITS)
    JoltEnkiTSJobSystem js_enkits(JPH::cMaxPhysicsJobs, JPH::cMaxPhysicsBarriers, &ts);
    job_system = &js_enkits;
#elif defined(QUEUE_BARRIER)
    JPH::JobSystemThreadPool thread_pool(JPH::cMaxPhysicsJobs, JPH::cMaxPhysicsBarriers, num_threads);
    JoltEnkiTSJobSystem js_barrier(JPH::cMaxPhysicsJobs, JPH::cMaxPhysicsBarriers, &ts);
    job_system = &js_barrier;
#elif defined(QUEUE_THREADPOOL)
    JPH::JobSystemThreadPool js_threadpool(JPH::cMaxPhysicsJobs, JPH::cMaxPhysicsBarriers, num_threads);
    job_system = &js_threadpool;
#endif

    // RUN 1: Pure Jolt
    std::cout << "[RUN 1] Pure Jolt Physics Pass...";
    ...
    double duration_jolt = std::chrono::duration<double, std::milli>(end_1 - start_1).count();
    std::cout << " Done.\n";

    std::vector<float> final_positions_1;
    final_positions_1.reserve(NUM_BODIES * 3);
    for (auto id : body_ids_1) {
        JPH::RVec3 pos = body_interface_1.GetPosition(id);
        final_positions_1.push_back(static_cast<float>(pos.GetX()));
        final_positions_1.push_back(static_cast<float>(pos.GetY()));
        final_positions_1.push_back(static_cast<float>(pos.GetZ()));
    }
    uint64_t hash_jolt = ComputeStateHash(final_positions_1.data(), final_positions_1.size() * sizeof(float));

    // RUN 2: Jolt + Highway SIMD
    std::cout << "[RUN 2] Jolt + Highway SIMD Pass...";
    ...
    double duration_hwy = std::chrono::duration<double, std::milli>(end_2 - start_2).count();
    std::cout << " Done.\n";

    std::vector<float> final_positions_2;
    final_positions_2.reserve(NUM_BODIES * 3);
    for (auto id : body_ids_2) {
        JPH::RVec3 pos = body_interface_2.GetPosition(id);
        final_positions_2.push_back(static_cast<float>(pos.GetX()));
        final_positions_2.push_back(static_cast<float>(pos.GetY()));
        final_positions_2.push_back(static_cast<float>(pos.GetZ()));
    }
    uint64_t hash_hwy = ComputeStateHash(final_positions_2.data(), final_positions_2.size() * sizeof(float));
    ...
    std::cout << "Pure Jolt Execution Time          : " << duration_jolt << " ms (" << duration_jolt / NUM_FRAMES << " ms/frame)\n";
    std::cout << "Jolt + Highway SIMD Execution Time: " << duration_hwy << " ms (" << duration_hwy / NUM_FRAMES << " ms/frame)\n";
    
    double speedup = ((duration_jolt - duration_hwy) / duration_jolt) * 100.0;
    std::cout << "Highway Speedup                   : " << (speedup >= 0 ? "+" : "") << speedup << "% faster throughput!\n";

    std::cout << "\n===============================================================\n";
    std::cout << "                DETERMINISM VERIFICATION REPORT                \n";
    std::cout << "===============================================================\n";
    std::cout << "Pure Jolt Hash: 0x" << std::hex << std::uppercase << hash_jolt << std::dec << "\n";
    std::cout << "Highway Hash  : 0x" << std::hex << std::uppercase << hash_hwy << std::dec << "\n";
