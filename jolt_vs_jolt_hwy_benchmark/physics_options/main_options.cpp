...
void ProcessHighwayParticleBatchEnkiTS(enki::TaskScheduler& ts, PhysicsParticleState* particles, size_t count, float dt) {
...
void ProcessScalarParticleBatch(PhysicsParticleState* particles, size_t count, float dt) {
...
void RunPhysicsOptionsTestPass(const std::string& testName, int velSteps, int posSteps, bool allowSleeping, float speculativeDist, enki::TaskScheduler& ts) {
    ...
    JoltEnkiTSJobSystem job_system(JPH::cMaxPhysicsJobs, JPH::cMaxPhysicsBarriers, &ts);

    JPH::PhysicsSettings settings;
    settings.mNumVelocitySteps = velSteps;
    settings.mNumPositionSteps = posSteps;
    settings.mAllowSleeping = allowSleeping;
    settings.mSpeculativeContactDistance = speculativeDist;

    // Pass A: Pure Jolt
    JPH::PhysicsSystem physics_system_1;
    physics_system_1.Init(static_cast<uint32_t>(NUM_BODIES + 10), 0, 2048, 2048, broad_phase_layer_interface, object_vs_broadphase_layer_filter, object_vs_object_layer_filter);
    physics_system_1.SetPhysicsSettings(settings);
    JPH::BodyInterface& body_interface_1 = physics_system_1.GetBodyInterface();
    ...
    body_interface_1.CreateAndAddBody(floor_bcs, JPH::EActivation::DontActivate);
    ...
    std::vector<JPH::BodyID> body_ids_1;
    body_ids_1.reserve(NUM_BODIES);
    ...
    for (size_t i = 0; i < NUM_BODIES; ++i) {
        ...
        JPH::Body* body = body_interface_1.CreateBody(sphere_bcs);
        body_ids_1.push_back(body->GetID());
        body_interface_1.AddBody(body->GetID(), JPH::EActivation::Activate);
    ...
    for (int frame = 0; frame < NUM_FRAMES; ++frame) {
        ProcessScalarParticleBatch(scalar_particles.data(), scalar_particles.size(), DELTA_TIME);
        physics_system_1.Update(DELTA_TIME, 1, &temp_allocator, &job_system);
    }
    auto end_1 = std::chrono::high_resolution_clock::now();
    double jolt_time = std::chrono::duration<double, std::milli>(end_1 - start_1).count();

    std::vector<float> final_pos_1;
    final_pos_1.reserve(NUM_BODIES * 3);
    for (auto id : body_ids_1) {
        JPH::RVec3 pos = body_interface_1.GetPosition(id);
        final_pos_1.push_back(static_cast<float>(pos.GetX()));
        final_pos_1.push_back(static_cast<float>(pos.GetY()));
        final_pos_1.push_back(static_cast<float>(pos.GetZ()));
    }
    uint64_t jolt_hash = ComputeStateHash(final_pos_1.data(), final_pos_1.size() * sizeof(float));

    // Pass B: Jolt + Highway SIMD
    JPH::PhysicsSystem physics_system_2;
    physics_system_2.Init(static_cast<uint32_t>(NUM_BODIES + 10), 0, 2048, 2048, broad_phase_layer_interface, object_vs_broadphase_layer_filter, object_vs_object_layer_filter);
    physics_system_2.SetPhysicsSettings(settings);
    JPH::BodyInterface& body_interface_2 = physics_system_2.GetBodyInterface();

    body_interface_2.CreateAndAddBody(floor_bcs, JPH::EActivation::DontActivate);

    std::vector<JPH::BodyID> body_ids_2;
    body_ids_2.reserve(NUM_BODIES);
    ...
    for (size_t i = 0; i < NUM_BODIES; ++i) {
        ...
        JPH::Body* body = body_interface_2.CreateBody(sphere_bcs);
        body_ids_2.push_back(body->GetID());
        body_interface_2.AddBody(body->GetID(), JPH::EActivation::Activate);
    ...
    for (int frame = 0; frame < NUM_FRAMES; ++frame) {
        ProcessHighwayParticleBatchEnkiTS(ts, highway_particles.data(), highway_particles.size(), DELTA_TIME);
        physics_system_2.Update(DELTA_TIME, 1, &temp_allocator, &job_system);
    }
    auto end_2 = std::chrono::high_resolution_clock::now();
    double hwy_time = std::chrono::duration<double, std::milli>(end_2 - start_2).count();

    std::vector<float> final_pos_2;
    final_pos_2.reserve(NUM_BODIES * 3);
    for (auto id : body_ids_2) {
        JPH::RVec3 pos = body_interface_2.GetPosition(id);
        final_pos_2.push_back(static_cast<float>(pos.GetX()));
        final_pos_2.push_back(static_cast<float>(pos.GetY()));
        final_pos_2.push_back(static_cast<float>(pos.GetZ()));
    }
    uint64_t hwy_hash = ComputeStateHash(final_pos_2.data(), final_pos_2.size() * sizeof(float));

    std::cout << std::left << std::setw(28) << testName 
              << std::right << std::setw(12) << jolt_time << " ms"
              << std::setw(12) << hwy_time << " ms"
              << "   0x" << std::hex << std::uppercase << jolt_hash << std::dec
              << "   " << ((jolt_hash == hwy_hash) ? "PASSED" : "FAILED") << "\n";
}
...
int main() {
    ...
    std::cout << "========================================================================================\n";
    std::cout << "                     PHYSICS ENGINE SOLVER & OPTIONS EXPLORATION BENCHMARK              \n";
    std::cout << "========================================================================================\n";
    std::cout << std::left << std::setw(28) << "Physics Configuration"
              << std::right << std::setw(15) << "Pure Jolt Time"
              << std::setw(15) << "Jolt+Hwy Time"
              << "   64-bit State Hash   Determinism\n";
    std::cout << "----------------------------------------------------------------------------------------\n";
    ...
    // Test 1: High Precision Solver (Vel=8, Pos=4)
    RunPhysicsOptionsTestPass("High Precision (V=8, P=4)", 8, 4, true, 0.02f, ts);

    // Test 2: Balanced Default (Vel=6, Pos=2)
    RunPhysicsOptionsTestPass("Balanced Default (V=6, P=2)", 6, 2, true, 0.02f, ts);

    // Test 3: Fast Throughput (Vel=2, Pos=1)
    RunPhysicsOptionsTestPass("Fast Throughput (V=2, P=1)", 2, 1, true, 0.02f, ts);

    // Test 4: Disable Sleeping (Force All Active)
    RunPhysicsOptionsTestPass("No Sleeping (V=6, P=2)", 6, 2, false, 0.02f, ts);

    // Test 5: Disable Speculative Contacts
    RunPhysicsOptionsTestPass("No Speculative Contacts", 6, 2, true, 0.0f, ts);
